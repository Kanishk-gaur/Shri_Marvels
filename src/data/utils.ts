...
export const generateCategories = (products: Product[]) => {
  ...
  const processCategory = (cat: Record<string, SubCategoryData>) => {
    const list = Object.values(cat).map((subcat) => {
      ...
      const uniqueSortedSizes = Array.from(new Set(subcat.sizes)).sort(
        sortSizes
      );

      // 2. Map the sorted original size names to their display names
      const displayedSizes = uniqueSortedSizes.map(
        (size) => sizeDisplayNames[size] || size
      );
    ...
    return list.sort((a, b) => {
      ...
      const originalNameA = Object.keys(priorityOrder).find(
        (key) =>
          subCategoryDisplayNames[key] === a.name ||
          key.toLowerCase().replace(/ /g, "-") === a.id
      );
      const originalNameB = Object.keys(priorityOrder).find(
        (key) =>
          subCategoryDisplayNames[key] === b.name ||
          key.toLowerCase().replace(/ /g, "-") === b.id
      );
