...
import { tilesProducts, categories, sizes } from "@/data/products"

const tilesCategories = categories.tiles.map((cat) => ({
  id: cat.id,
  name: cat.name,
  sizes: sizes.tiles,
}))
...
export default function TilesPage() {
  ...
  const filteredProducts = tilesProducts
    .filter((product) => {
      if (!selectedCategory) return true
      return product.subcategory.toLowerCase() === selectedCategory
    })
    .filter((product) => {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-6 space-y-4 lg:space-y-0">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-white/60">({filteredProducts.length} products)</span>
            </div>
